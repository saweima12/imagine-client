...
export default ({ curPath, list }: { curPath: string, list: FileStat[] } ) => {
    ...
    // declare state.
    const checkedList = useSyncExternalStore(
        checkedListStore.subscribe, 
        checkedListStore.getState
    )
    
    const onChangeCheckAll = (e: ChangeEvent) => {
        const newState = (e.target as HTMLInputElement).checked;
        if (newState) 
            checkedListStore.setState(fileList.map(item => item.filename));
        else
            checkedListStore.setState([]);
    }

    const onChangeItem = (e: ChangeEvent, item: FileStat) => {
        const newState = (e.target as HTMLInputElement).checked;
        if (newState)
            checkedListStore.setState([ ...checkedList, item.filename]);
        else
            checkedListStore.setState(checkedList.filter(name => name != item.filename));
    }

    return <div className="flex flex-col filelist-view">
                ...
                <div className="flex row" key={item.filename}>
                    <div className="multi-check">
                        <input type="checkbox"
                            ...
                            onChange={e => onChangeItem(e, item)}></input>
                    </div>
                    <div className="flex file-name">
                        <FileListIcon type={item.type}/>
                        {
                            item.type == "directory" ? (
                                <NavLink to={`${item.filename.replace(/^\//, "")}`}>
                                    {item.basename}
                                </NavLink>
                            ):(
                                <p>{item.basename}</p>
