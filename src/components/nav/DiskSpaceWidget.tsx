...
export default () => {
    ...
    const sysInfo = useSyncExternalStore(
        sysInfoStore.subscribe, 
        sysInfoStore.getState,
    );
    ...
    const percent = useMemo(() => {
        if (sysInfo.disk)
            return Math.round((sysInfo.disk.usage / sysInfo.disk.all) * 100) ;
        return 1;
    }, [sysInfo.disk?.usage])

    const usage_str = useMemo(() => {
        if (sysInfo.disk) 
            return formatSizeString(sysInfo.disk.usage);
        return 0;
    }, [sysInfo.disk?.usage])
    ...
    return <div className="py-4 diskinfo-wrapper">
